...
import { requireRole, ROLE_RANK } from '@/lib/auth/guards'
...
export async function changeUserRole(
  ...
  await logAudit(caller.id, 'profiles', targetUserId, 'user_role_changed',
    { role: target.role },
    { role: newRole }
  )
...
export async function inviteUserAction(
  ...
  await logAudit(caller.id, 'profiles', email, 'user_invited', null, { email })
  revalidatePath('/configuracoes/usuarios')
  return { ok: true }
}
