...
async function classifyQuestion(questionId: string): Promise<void> {
  ...
  const systemPrompt = `Você é um classificador de questões médicas.
...
E) ${alternatives['E'] ?? ''}`
  ...
  if (tagInsertError) {
    console.error(`[classify ${questionId}] Insert tags falhou: ${tagInsertError.message}`)
  }
}
...
export async function generateComment(questionId: string): Promise<void> {
  ...
  const prompt = `Você é um professor de Medicina escrevendo um comentário didático completo para uma questão de prova. Este comentário é o material de estudo principal do aluno — precisa ser denso, claro e cobrir TODAS as alternativas individualmente.
...
- Use os nomes próprios das condutas/diretrizes.
- NÃO use markdown (sem **, sem ##, sem listas numeradas além dos bullets dos pontos-chave).
- NÃO inclua título do comentário nem encerramento.
...
Retorne APENAS o texto do comentário seguindo a estrutura acima.`
  ...
  if (commentInsertError) {
    console.error(`[comment ${questionId}] Insert falhou: ${commentInsertError.message}`)
  }
}
...
export async function runExtractionPipeline(exam_id: string): Promise<void> {
  ...
  const { data: exam, error: examError } = await supabase
    .from('exams')
    .select('id, source_pdf_path, year, booklet_color, extractor_id, specialty_id, specialties(slug)')
    .eq('id', exam_id)
    .single()
  ...
  // ── Detecção de banca ────────────────────────────────────────────────────
  await setProgress(exam_id, 'detecting_banca', 0, 1, 'Identificando banca da prova')

  const examExtractorId = (exam.extractor_id as string | null) ?? null
  let banca: BancaParser
  let bancaSource: 'manual' | 'auto' = 'manual'

  let textResultPromise: ReturnType<typeof extractTextFirst> | null = null

  if (examExtractorId) {
    banca = getBancaPorId(examExtractorId)
  } else {
    // Para detectar a banca precisamos do texto. Roda pdftotext rápido
    // antes do text-first; o resultado completo virá depois.
    const { runPdftotext } = await import('./text-first')
    let detectionText = ''
    try {
      detectionText = await runPdftotext(pdfBuffer, true)
    } catch {
      detectionText = ''
    }
    const detected = detectarBanca(detectionText)
    banca = detected.banca
    bancaSource = 'auto'
    await supabase.from('exams').update({ extractor_id: banca.id }).eq('id', exam_id)
    console.log(
      `[extract ${exam_id}] Extrator detectado: ${banca.id} (score=${detected.score.toFixed(2)})`
    )
  }
